...
class GenSampler():
    ...
    def compute_m_part(self, smiles, full_molecule=True):
        ...
        # 批量规则特征
        if full_molecule:
            rule_features = compute_m_part_rule_features(smiles)  # shape=(batch, n_rule_features)
        else:
            rule_features = np.zeros((len(smiles), 2053), dtype=np.float32)

        # 批量生成器表征（如支持批量，建议直接用；否则循环处理）
        try:
            pooled_reprs = self.model.get_pooled_repr(smiles)  # shape=(batch, d_model)
            ...
            pooled_reprs = np.stack(pooled_reprs)

        # 拼接特征
        m_parts = np.concatenate([rule_features, pooled_reprs], axis=1)  # shape=(batch, n_rule_features+d_model)
    ...
    def train_surrogate(self, epochs=2, batch_size=32):
        ...
        if len(self.m_part_buffer) < batch_size:
            ...
            return
        losses = []
        m_parts = torch.tensor([x[0] for x in train_data], dtype=torch.float32)
        rewards = torch.tensor([x[1] for x in train_data], dtype=torch.float32)
        dataset = torch.utils.data.TensorDataset(m_parts, rewards)
        loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)
        for epoch in range(epochs):
            epoch_loss = 0.0
            num_batches = 0
            for batch_m_parts, batch_rewards in loader:
    ...
    def sample(self, data=None, use_replay=False):
        ...
        支持批次操作，Surrogate和reward_fn均批量处理
        """
        self.model.eval()
            ...
            for i in range(init, self.max_len):
                ...
                if self.enable_surrogate and self.surrogate_intervention and self.surrogate:
                    ...
                    if completed_indices:
                        ...
                        # 批量计算M_part
                        m_parts = self.compute_m_part(completed_smiles, full_molecule=True)
        ...
        # 只对完整分子计算M_part和训练Surrogate（批量处理）
        if self.enable_surrogate and self.surrogate:
            valid_smiles = [smi for smi in smiles if len(smi) > 3]  # 可调整长度阈值
            m_parts = self.compute_m_part(valid_smiles, full_molecule=True)
...
class Surrogate(nn.Module):
    ...
    def mc_predict(self, m_part, n_samples=10):
        ...
        import torch
        import numpy as np
        # 支持 list、numpy、tensor 输入
        if isinstance(m_part, list):
            m_part = np.array(m_part, dtype=np.float32)
        if isinstance(m_part, np.ndarray):
            m_part = torch.tensor(m_part, dtype=torch.float32)
        device = next(self.parameters()).device
        m_part = m_part.to(device)
        # 批量复制输入，直接一次性推理
        m_part_batch = m_part.unsqueeze(0).repeat(n_samples, 1, 1)  # [n_samples, batch_size, input_dim]
        m_part_batch = m_part_batch.view(-1, m_part.shape[-1])      # [n_samples*batch_size, input_dim]
        with torch.no_grad():
            pred = self(m_part_batch)
        pred = pred.view(n_samples, -1)  # [n_samples, batch_size]
