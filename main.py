...
# ===========================
# Surrogate参数
parser.add_argument('--surrogate_threshold', type=float, default=0.2, help='Surrogate分数阈值，越高筛选越严格，分子多样性和数量可能下降')
parser.add_argument('--surrogate_uncertainty_threshold', type=float, default=0.5, help='Surrogate不确定性阈值，越低筛选越严格')
    ...
    if len(generated_mols) == 0:
        ...
        if len(novel_smiles):
            ...
            if epoch is not None and time is not None:
                with open(PROPERTY_FILE, 'a+') as wf:
                    wf.write('{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.5f},{:.3f}\n'.format(
                        epoch+1,
...
def main():
    ...
    sampler = GenSampler(
        ...
        surrogate_intervention=True,
        surrogate_threshold=args.surrogate_threshold,       # 使用命令行参数
        surrogate_uncertainty_threshold=args.surrogate_uncertainty_threshold # 使用命令行参数
    )
    ...
    if args.adversarial_train:
        ...
        for epoch in range(args.adv_epochs): 
            rollsampler = GenSampler(rollout.own_model, gen_data_loader.tokenizer, args.batch_size, args.max_len, reward_fn=reward_fn, buffer_capacity=args.buffer_capacity, replay_prob=args.replay_prob, m_part_dim=2181,                # M_part特征维度（根据你的特征拼接实际设置）
            enable_surrogate=True,         # 启用Surrogate
            surrogate_intervention=True,   # 生成过程中启用Surrogate干预
            surrogate_threshold=0.2,       # 分子片段分数阈值 0.5
            surrogate_uncertainty_threshold=0.5) # 不确定性阈值 0.2
            ...
            # Generate Samples
            print('Generating {} samples...'.format(args.generated_num))
            sampler = GenSampler(gen, gen_data_loader.tokenizer, args.batch_size, args.max_len, reward_fn=reward_fn, buffer_capacity=args.buffer_capacity, replay_prob=args.replay_prob, m_part_dim=2181,
            enable_surrogate=True,         # 启用Surrogate
            surrogate_intervention=True,   # 生成过程中启用Surrogate干预
            surrogate_threshold=0.2,       # 分子片段分数阈值 0.5
            surrogate_uncertainty_threshold=0.5) # 不确定性阈值 0.2)
